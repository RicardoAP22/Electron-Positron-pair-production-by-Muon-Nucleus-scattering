...
void Histo::Save()
{
    ...
    // Escribir archivo ROOT
    if (!fManager->Write()) {
        G4cout << "Histo::Save: FATAL ERROR writing ROOT file" << G4endl;
        exit(1);
    }

    // Obtener número total de histogramas
    G4int nHist = fManager->GetNofH1s();

    // Recorrer y exportar cada histograma
    for (G4int i = 0; i < nHist; i++) {
        // Obtener el histograma
        auto histo = fManager->GetH1(i);
        if (!histo) continue;

        // Crear archivo CSV para cada histograma
        G4String filename = "Histogram_" + std::to_string(i) + ".csv";
        ...
        // Escribir encabezado con columna de errores
        outFile << "BinCenter,Content,Error\n";

        // Recorrer cada bin del histograma
        for (G4int bin = 0; bin < histo->axis().bins(); bin++) {
            G4double binCenter = histo->axis().bin_center(bin);
            G4double binContent = histo->bin_height(bin);
            G4double binError = histo->bin_error(bin);  // Obtener error del bin
            outFile << binCenter << "," << binContent << "," << binError << "\n";
        }

        outFile.close();
        G4cout << "Histograma guardado en " << filename << G4endl;
    }
